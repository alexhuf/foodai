...
def evaluate_supervised_model(
    ...
    for batch in loader:
        ...
        _, parts = compute_supervised_loss(batch_gpu, outputs, class_meta, recon_weight, class_weight, reg_weight)
        losses.append(parts)
...
def train_space(
    ...
    run_dir = project_root / "models" / "representation_v3" / space
    ensure_dir(run_dir)
    ...
    if mode in {"supervised_context", "supervised_structure"}:
        encoded = encode_supervised_targets(df, cfg["class_targets"], cfg["reg_targets"], min_class_count=5)
...
def main():
    parser = argparse.ArgumentParser(description="Train representation encoders v3 with checkpoint/resume across meal + regime spaces.")
    ...
    ensure_dir(project_root / "models" / "representation_v3")
