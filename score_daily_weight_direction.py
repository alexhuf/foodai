...
def infer_feature_row_from_history(source_row_df: pd.DataFrame, full_transition_df: pd.DataFrame) -> pd.DataFrame:
    # Rebuild features exactly like training from full transition-like frame,
    # then select the one row matching source row period_id.
...
def build_interpretation(target: str, primary_channel: str, primary_prob: float, band: str, top_local: pd.DataFrame) -> str:
    ...
    if feature_bits:
        return f"{intro} {channel_text} Local driver proxies suggest: " + "; ".join(feature_bits) + "."
    return f"{intro} {channel_text}"
...
def score_one_target(
    ...
    trainer_meta = load_json(reports_root / "meta.json")
    ...
    # Clean calibration channel: fit train-only base model, fit calibrators on val, score latest row.
