...
def dedupe_columns(df: pd.DataFrame) -> pd.DataFrame:
    # Keep the first occurrence of any repeated column label.
    return df.loc[:, ~pd.Index(df.columns).duplicated(keep="first")].copy()


def cols_present(df: pd.DataFrame, wanted: list[str]) -> list[str]:
...
def build_final_tables(project_root: Path, meal_db_dir: str = "meal_db") -> None:
    ...
    alias = dedupe_columns(read_csv_required(alias_path))
    entity = dedupe_columns(read_csv_required(entity_path))
    comp = dedupe_columns(read_csv_required(component_path))
    event = dedupe_columns(read_csv_required(event_path))
    raw_entry = dedupe_columns(read_csv_required(raw_entry_path))
    ...
    if "llm_review_status" in entity_final.columns:
        entity_final["is_reviewed"] = (
            entity_final["llm_review_status"].fillna("").astype(str).str.lower().eq("done")
        )
    else:
        entity_final["is_reviewed"] = False

    if "canonical_display_name" in entity_final.columns and "provisional_display_name" in entity_final.columns:
        entity_final["canonical_display_name"] = entity_final["canonical_display_name"].fillna(
            entity_final["provisional_display_name"]
        )
    ...
    entity_final = dedupe_columns(entity_final[keep_entity_cols])

    # ---------- meal_component_edge ----------
    comp_final = comp.copy()

    if "canonical_entity_id" not in comp_final.columns and "provisional_canonical_entity_id" in comp_final.columns:
        comp_final["canonical_entity_id"] = comp_final["provisional_canonical_entity_id"]
    elif "canonical_entity_id" in comp_final.columns and "provisional_canonical_entity_id" in comp_final.columns:
        comp_final["canonical_entity_id"] = comp_final["canonical_entity_id"].fillna(
            comp_final["provisional_canonical_entity_id"]
        )

    entity_lookup_cols = cols_present(
        entity_final,
        [
            "canonical_entity_id",
            "canonical_display_name",
    ...
    comp_final = comp_final.merge(
        dedupe_columns(entity_final[entity_lookup_cols]).drop_duplicates(subset=["canonical_entity_id"]),
    ...
    comp_final = dedupe_columns(comp_final[keep_comp_cols])

    # ---------- meal_semantic_features ----------
    event_final = event.copy()
    if "llm_review_status" in event_final.columns:
        event_final["is_semantically_reviewed"] = (
            event_final["llm_review_status"].fillna("").astype(str).str.lower().eq("done")
        )
    else:
        event_final["is_semantically_reviewed"] = False

    comp_summary = (
        comp_final.groupby("meal_id", dropna=False)
        .agg(
    ...
    event_final = dedupe_columns(event_final[keep_event_cols])

    # ---------- food_entry_semantic_view ----------
    raw_final = raw_entry.copy()

    alias_lookup_cols = cols_present(
        alias,
        [
            "alias_id",
            "canonical_entity_id",
    ...
    alias_lookup = dedupe_columns(alias[alias_lookup_cols]).drop_duplicates(subset=["alias_id"])
    raw_final = dedupe_columns(raw_final)
    raw_final = raw_final.merge(alias_lookup, on="alias_id", how="left", suffixes=("", "_alias"))
    raw_final = dedupe_columns(raw_final)

    if "canonical_entity_id" not in raw_final.columns and "provisional_canonical_entity_id" in raw_final.columns:
        raw_final["canonical_entity_id"] = raw_final["provisional_canonical_entity_id"]
    elif "canonical_entity_id" in raw_final.columns and "provisional_canonical_entity_id" in raw_final.columns:
        raw_final["canonical_entity_id"] = raw_final["canonical_entity_id"].fillna(
            raw_final["provisional_canonical_entity_id"]
        )

    entity_join_cols = cols_present(
        entity_final,
        [
            "canonical_entity_id",
            "canonical_display_name",
    ...
    entity_join = dedupe_columns(entity_final[entity_join_cols]).drop_duplicates(subset=["canonical_entity_id"])
    raw_final = raw_final.merge(entity_join, on="canonical_entity_id", how="left")
    raw_final = dedupe_columns(raw_final)

    comp_role_lookup = dedupe_columns(
        comp_final[cols_present(comp_final, ["food_entry_uuid", "component_role_final", "role_confidence"])]
    ).drop_duplicates(subset=["food_entry_uuid"])
    ...
    raw_final = dedupe_columns(raw_final[keep_raw_cols])
