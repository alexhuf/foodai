...
DAILY_CONFIG = DailyConfig(
    ...
    key_numeric_targets=[
        ...
        "samsung_stress_mean_day",
        "samsung_resting_hr_day",
    ],
...
def find_existing_path(project_root: Path, candidates: Sequence[str]) -> Path:
    ...
    raise FileNotFoundError(
        "Could not find any expected daily summary matrix. Tried: "
...
def infer_day_id(df: pd.DataFrame, id_col: Optional[str], time_col: str) -> pd.Series:
    ...
    return t.dt.strftime("%Y-%m-%d").fillna(df.index.astype(str))
...
def summarize_transition_table(df: pd.DataFrame, meta: Dict, source_csv: Path) -> Dict:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build forward-looking daily transition targets.")
    parser.add_argument("--project-root", default=".", help="Path to project root.")
    parser.add_argument("--source-csv", default="", help="Optional explicit relative path to daily summary matrix.")
    ...
    log(f"Loading daily summary matrix from: {source_csv}")
    ...
    summary = summarize_transition_table(transition_df, meta, source_csv)
