...
def load_target_specs(spec_json: Dict, target_group: str) -> List[TargetSpec]:
    targets = [TargetSpec(**t) for t in spec_json["targets"]]
    if target_group == "primary":
        targets = [t for t in targets if t.name in PRIMARY_TARGETS]
    return targets


def encode_class_target(y: pd.Series, min_class_count: int = 5) -> Tuple[np.ndarray, Dict[str, int], List[str], pd.Series]:
    raw = y.astype("object")
...
def train_one_target(
    ...
    model_dir = model_root / target.name
    report_dir = reports_root / target.name
    fi_dir = fi_root / target.name
    ...
    if target.kind in CLASSIFICATION_KINDS:
        enc_all, label_to_idx, labels, text_series = encode_class_target(df[target.name], min_class_count=5)
        train_mask = enc_all[train_df.index] >= 0
        val_mask = enc_all[val_df.index] >= 0
        test_mask = enc_all[test_df.index] >= 0

        if train_mask.sum() < 20 or len(np.unique(enc_all[train_df.index][train_mask])) < 2:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train daily transition baselines.")
    ...
    parser.add_argument("--target-spec-json", default="training/daily_transition/days_transition_target_spec.json", help="Relative path to daily target spec.")
    ...
    transition_csv = transition_dir / "days_transition_matrix.csv"
    if not transition_csv.exists():
        raise FileNotFoundError(f"Missing daily transition matrix: {transition_csv}")
    ...
    model_root = project_root / "models" / "daily_transition"
    reports_root = project_root / "reports" / "backtests" / "daily_transition"
    fi_root = project_root / "reports" / "feature_importance" / "daily_transition"
    ensure_dir(model_root)
    ensure_dir(reports_root)
    ensure_dir(fi_root)

    targets = load_target_specs(spec_json, args.target_group)
    log(f"Loading daily transition matrix and target spec ...")
    log(f"Training {len(targets)} targets ({args.target_group}) ...")
    ...
    for target in targets:
        ...
        if res is not None:
            ...
            log(f"Finished {target.name} (best={res['best_model']})")
        else:
            log(f"Skipped {target.name}")
    ...
    log(f"Backtests written under: {reports_root}")
