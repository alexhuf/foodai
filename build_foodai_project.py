...
def find_source_file(project_root: Path, source_name: str, pattern: str) -> Optional[Path]:
    ...
    # Fallback for testing or flat layouts.
    path = find_first_csv(project_root, pattern)
    if path is not None:
        return path
...
def clean_wall_clock_string(value: Any) -> Optional[str]:
    ...
    if not s:
        return None
    # Remove bracketed timezone names like [America/New_York]
    s = re.sub(r"\[[^\]]+\]", "", s)
    return s
...
def to_datetime_local(value: Any) -> pd.Timestamp:
    ...
    try:
        ...
        if pd.isna(ts):
            return pd.NaT
        # Keep local wall-clock time and drop timezone info if present.
...
def split_top_level_csvish(text: str) -> List[str]:
    text = text.strip()
...
def parse_values_keys_row(row: pd.Series) -> Dict[str, Any]:
    keys_raw = split_top_level_csvish(str(row.get("keys", "")))
    values_raw = split_top_level_csvish(str(row.get("values", "")))
    keys = [k.strip() for k in keys_raw]
...
def parse_values_keys_export(df: pd.DataFrame) -> pd.DataFrame:
    parsed_rows = [parse_values_keys_row(row) for _, row in df.iterrows()]
...
def build_samsung_weight_events(path: Path) -> pd.DataFrame:
    ...
    out = out.dropna(subset=["datetime_local", "weight_kg"]).drop_duplicates(subset=["datetime_local", "weight_kg", "deviceuuid"])
...
def build_samsung_steps_intraday(path: Path) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "calorie_kcal": pd.to_numeric(df.get("com.samsung.health.step_count.calorie"), errors="coerce"),
...
def build_samsung_energy_daily(path: Path) -> pd.DataFrame:
    df = read_samsung_csv(path)
    day_ms = pd.to_numeric(df.get("com.samsung.shealth.calories_burned.day_time"), errors="coerce")
    out = pd.DataFrame({
        "date": pd.to_datetime(day_ms, unit="ms", errors="coerce").dt.floor("D"),
...
def build_noom_food_entries(path: Path) -> pd.DataFrame:
    ...
    out = out.dropna(subset=["date"]).sort_values(["date", "time_slot", "client_time_inserted", "server_time_created"]).reset_index(drop=True)
    return out
...
def build_noom_daily_budgets(path: Path) -> pd.DataFrame:
    ...
    df = parse_values_keys_export(raw)
    out = pd.DataFrame({
        "date": pd.to_datetime(df.get("date"), errors="coerce").dt.floor("D"),
...
def build_noom_goals(path: Path) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "completion_timestamp": goal_json.map(
            lambda x: to_datetime_local(x.get("goalStats", {}).get("completionTimestamp")) if isinstance(x, dict) else pd.NaT
        ),
    })
    out["source"] = "noom"
...
def build_samsung_daily_features(
    ...
    weight_daily = (
        samsung_weight_events.groupby("date", as_index=False)
        .agg(
    ...
    sleep_daily = (
        samsung_sleep_sessions.groupby("date", as_index=False)
        .agg(
    ...
    exercise_daily = (
        samsung_exercise_sessions.groupby("date", as_index=False)
        .agg(
...
def build_noom_daily_features(
    ...
    food_daily = (
        noom_food_entries.groupby("date", as_index=False)
        .agg(
    ...
    meal_daily = (
        noom_meal_events.groupby("date", as_index=False)
        .agg(
    ...
    weigh_daily = (
        noom_weighins.groupby("date", as_index=False)
        .agg(
    ...
    steps_daily = (
        noom_steps_daily.groupby("date", as_index=False)
        .agg(noom_steps=("steps", "max"))
    )

    water_daily = (
        noom_water_logs.groupby("date", as_index=False)
        .agg(noom_water_liters=("water_liters", "sum"))
    )

    app_daily = (
        noom_app_opens.groupby("date", as_index=False)
        .agg(noom_app_open_count=("event_type", "count"))
    )

    budget = noom_daily_budgets.drop_duplicates(subset=["date"], keep="last").copy()
...
def build_master_event_ledger(
    ...
    app["details"] = None
    parts.append(app.rename(columns={"datetime_local": "datetime_local"})[["source", "event_type", "datetime_local", "date", "metric_1", "metric_1_name", "details"]])

    sw = samsung_weight_events.copy()
    sw["datetime_local"] = sw["datetime_local"]
...
def add_weight_trends(master_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    fused_weight_lb = out["noom_weight_lb"].combine_first(out["samsung_weight_lb"])
    out["true_weight_lb"] = fused_weight_lb
...
def build_master_daily_features(
    samsung_daily_features: pd.DataFrame,
    noom_daily_features: pd.DataFrame,
) -> pd.DataFrame:
    out = samsung_daily_features.merge(noom_daily_features, on="date", how="outer")
    out = out.sort_values("date").reset_index(drop=True)
    out = add_weight_trends(out)
    return out
...
def resample_intraday(df: pd.DataFrame, time_col: str, value_cols: Dict[str, str], freq: str = "15min") -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame()
    temp = df.copy()
    temp = temp.dropna(subset=[time_col]).set_index(time_col).sort_index()
...
def build_master_15min_telemetry_active(
    ...
    master_daily_features: pd.DataFrame,
) -> pd.DataFrame:
    if noom_meal_events.empty:
        raise ValueError("No meal events found. Cannot build active telemetry window.")

    first_meal = pd.to_datetime(noom_meal_events["date"]).min()
    last_meal = pd.to_datetime(noom_meal_events["date"]).max()

    start = first_meal - pd.Timedelta(days=30)
    end = last_meal + pd.Timedelta(days=1)
    ...
    meal_15 = meal_15.rename(columns={
    ...
    out["date"] = out["datetime_local"].dt.floor("D")
    out = out.merge(master_daily_features, on="date", how="left")
...
def build_all(project_root: Path) -> None:
    ...
    master_event_ledger = build_master_event_ledger(
        noom_meal_events,
        noom_weighins,
        noom_app_opens,
        samsung_weight_events,
    ...
    master_daily_features = build_master_daily_features(samsung_daily_features, noom_daily_features)
...
def main() -> None:
    ...
    parser.add_argument(
        "--project-root",
        default=".",
        help="Path to the project root containing samsung/ and noom/ folders.",
    )
