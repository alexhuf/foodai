...
def log(msg: str) -> None:
    print(f"[daily-transition-v3] {msg}")
...
def build_transition_table(
    ...
    meta = {
        ...
        "weight_target_definition": "y_next_weight_delta_lb = true_weight_lb(t+1) - true_weight_lb(t)",
    }
    return out, meta
...
def build_target_spec(df: pd.DataFrame, meta: Dict) -> Dict:
    ...
    # Direct D+1 weight target.
    maybe_add(
        "y_next_weight_delta_lb",
        "regression",
        "true_weight_lb",
        "true_weight_lb",
        "Direct next-day weight change in lb: true_weight_lb(t+1) - true_weight_lb(t).",
        notes="This is intentionally D+1 aligned and is not shifted a second time.",
    )
    ...
    maybe_add("y_next_weight_loss_flag", "binary_classification", "true_weight_lb", "true_weight_lb", "Whether next-day direct weight change is <= -0.5 lb.")
    maybe_add("y_next_weight_gain_flag", "binary_classification", "true_weight_lb", "true_weight_lb", "Whether next-day direct weight change is >= 0.5 lb.")
    ...
    return {
        ...
        "weight_target_definition": meta["weight_target_definition"],
        "targets": targets,
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Build forward-looking daily transition targets with D+1 weight alignment.")
