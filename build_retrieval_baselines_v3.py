...
# Structure-only weekly/weekend spaces: avoid direct outcome columns like weight_delta.
WEEK_STRUCTURE_FEATURES = [
    "n_days",
    "n_weekend_days",
    "days_with_meals",
    "days_with_weight",
...
def fit_embedding_pipeline(X: pd.DataFrame, max_components: int):
    ...
    # Always apply TruncatedSVD once dimension reduction is requested and possible,
    # even if Xt is dense. This fixes the v2 issue where dense matrices stayed huge.
    ...
    if n_components >= 2:
        ...
        embedding = svd.fit_transform(Xt_arr)
...
def build_space(project_root: Path, spec: Dict, top_k: int = 10) -> None:
    ...
    out_dir = project_root / "models" / "retrieval_v3" / name
    ensure_dir(out_dir)
    ...
    for c in ["date", "decision_time", "week_start", "week_end", "weekend_start", "weekend_end", "datetime_local_approx"]:
        if c in df.columns:
...
def meal_context_spec(project_root: Path) -> Dict:
    ...
    return {
        ...
        "max_components": 48,
        "notes": "Context-state meal retrieval for similar situations before a meal decision.",
    }
...
def meal_semantic_spec(project_root: Path) -> Dict:
    return {
        ...
        "max_components": 24,
        "notes": "Semantic meal retrieval for adjacent meals, novelty, and candidate generation.",
    }
...
def day_spec(project_root: Path) -> Dict:
    return {
        ...
        "max_components": 24,
        "notes": "Curated day retrieval focused on behaviorally meaningful day similarity.",
    }


def week_structure_spec(project_root: Path) -> Dict:
    return {
        "name": "weeks_structure",
        "path": project_root / "training" / "week_summary_matrix.csv",
        "id_col": "week_id",
        "feature_cols": WEEK_STRUCTURE_FEATURES,
        "label_cols": [
            "week_start",
            "week_end",
            "week_label",
        ...
        "max_components": 12,
        "notes": "Structure-only weekly retrieval excluding direct outcome fields like weight delta from the similarity features while still surfacing them in labels for explanation.",
    }


def weekend_structure_spec(project_root: Path) -> Dict:
    return {
        "name": "weekends_structure",
        "path": project_root / "training" / "weekend_summary_matrix.csv",
        "id_col": "weekend_id",
        "feature_cols": WEEKEND_STRUCTURE_FEATURES,
        "label_cols": [
            "weekend_start",
            "weekend_end",
            "weekend_label",
        ...
        "max_components": 12,
        "notes": "Structure-only weekend retrieval excluding direct outcome fields from the similarity features.",
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Build retrieval baselines v3 with forced compression and structure-only week/weekend spaces.")
    ...
    ensure_dir(project_root / "models" / "retrieval_v3")

    specs = [
        meal_context_spec(project_root),
        meal_semantic_spec(project_root),
        day_spec(project_root),
        week_structure_spec(project_root),
        weekend_structure_spec(project_root),
    ]
