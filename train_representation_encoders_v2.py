...
SPACE_CONFIGS = {
    ...
    "meal_target_semantics": {
        ...
        # Eval labels only, not supervised heads.
        "purity_labels": [
...
def encode_supervised_targets(df: pd.DataFrame, class_targets: List[str], reg_targets: List[str], min_class_count: int = 10) -> EncodedTargets:
    class_arrays: Dict[str, np.ndarray] = {}
    class_meta: Dict[str, Dict] = {}
    reg_arrays: Dict[str, np.ndarray] = {}
...
class BaseTabularDataset(Dataset):
    def __init__(
        ...
        reg_targets: Optional[Dict[str, np.ndarray]] = None,
    ):
        self.X = X.astype(np.float32)
        self.ids = ids
        self.split_labels = split_labels
        self.class_targets = class_targets or {}
        self.reg_targets = reg_targets or {}
    ...
    def __getitem__(self, idx):
        ...
        for name, arr in self.reg_targets.items():
            val = arr[idx]
            out[f"reg::{name}"] = torch.tensor(val if np.isfinite(val) else np.nan, dtype=torch.float32)
        return out
...
class SupervisedContextModel(nn.Module):
    def __init__(
        self,
        input_dim: int,
        class_meta: Dict[str, Dict],
...
def compute_context_loss(
    ...
    for name in outputs["reg_out"].keys():
        ...
        if mask.sum() == 0:
            continue
        mse = nn.functional.mse_loss(outputs["reg_out"][name][mask], y[mask])
...
@torch.no_grad()
def extract_latents(model: nn.Module, loader: DataLoader, device: torch.device, mode: str) -> Tuple[np.ndarray, List[str], List[str]]:
    model.eval()
    zs: List[np.ndarray] = []
    ids: List[str] = []
    splits: List[str] = []
...
def cosine_knn(query: np.ndarray, ref: np.ndarray, k: int = 5) -> np.ndarray:
    # Normalize then brute-force cosine similarity
...
def evaluate_context_model(
    ...
    for batch in loader:
        ...
        _, parts = compute_context_loss(batch_gpu, outputs, class_meta, recon_weight, class_weight, reg_weight)
        losses.append(parts)
...
def export_latents(model: nn.Module, dataset: BaseTabularDataset, device: torch.device, out_csv: Path):
    ...
    z, ids, splits = extract_latents(model, loader, device=device, mode="latent")
    rows = []
...
def train_space(
    ...
    run_dir = project_root / "models" / "representation_v2" / space
    ensure_dir(run_dir)
    ...
    if mode == "supervised_context":
        encoded = encode_supervised_targets(df, cfg["class_targets"], cfg["reg_targets"], min_class_count=10)
        model = SupervisedContextModel(
        ...
        dataset = BaseTabularDataset(X_np, ids, split_labels, encoded.class_arrays, encoded.reg_arrays)
        class_meta = encoded.class_meta
        reg_names = list(encoded.reg_arrays.keys())
    else:
        ...
        dataset = BaseTabularDataset(X_np, ids, split_labels, None, None)
        class_meta = {}
        reg_names = []
            ...
            for batch in train_loader:
                ...
                if mode == "supervised_context":
                    ...
                    with AutocastContext(enabled=use_amp):
                        ...
                        loss, parts = compute_context_loss(
                            batch_gpu,
                            outputs,
    ...
    # Test evaluation
    if mode == "supervised_context":
        test_metrics = evaluate_context_model(
            model=model,
            loader=test_loader,
            device=device,
            class_meta=class_meta,
    ...
    # Latent evaluation
    full_loader = DataLoader(dataset, batch_size=512, shuffle=False, num_workers=0, collate_fn=collate_fn)
    z_all, id_all, split_all = extract_latents(model, full_loader, device=device, mode=mode)
    ...
    if train_mask.sum() > 0 and val_mask.sum() > 0:
        latent_eval["val"] = neighbor_purity_metrics(
            ...
            label_cols=cfg.get("purity_labels", []),
            k=5,
        )
...
def main():
    parser = argparse.ArgumentParser(description="Train representation encoders v2 with checkpoint/resume.")
    ...
    ensure_dir(project_root / "models" / "representation_v2")
