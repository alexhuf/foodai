...
def longify_sequence(
    ...
    base_cols = _dedupe(["anchor_id", "modality", "step_back", "sequence_time", "age_days"] + keep_cols)

    if rows_df.empty:
        return pd.DataFrame(columns=base_cols)

    # If upstream logic accidentally passed duplicate column names, keep the first occurrence only.
    rows_df = rows_df.loc[:, ~rows_df.columns.duplicated()].copy()
    ...
    cols = [c for c in base_cols if c in rows_sorted.columns]
...
def main() -> None:
    ...
    for i, (_, anchor) in enumerate(anchors.iterrows()):
        ...
        day_mask[i, :] = Md
        day_age[i, :] = Ad
        keep_day_cols = list(dict.fromkeys(["period_id", "period_start"] + day_num_cols[:40] + day_cat_cols[:20]))
        ...
        if week_df is not None:
            ...
            week_mask[i, :] = Mw
            week_age[i, :] = Aw
            keep_week_cols = list(dict.fromkeys(["period_id", "period_start"] + week_num_cols[:40] + week_cat_cols[:20]))
        ...
        if meal_df is not None and meal_time_col is not None:
            ...
            meal_mask[i, :] = Mm
            meal_age[i, :] = Am
            keep_meal_cols = list(dict.fromkeys([meal_id_col, meal_time_col] + meal_num_cols[:40] + meal_cat_cols[:20]))
