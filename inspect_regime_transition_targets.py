...
def extract_feature_drivers(pipe, top_k: int = 20) -> Tuple[pd.DataFrame, str]:
    ...
    if hasattr(model, "coef_"):
        ...
        if coef.ndim == 2 and coef.shape[0] == 1:
            ...
            # Multiclass: use mean absolute coefficient magnitude across classes.
...
def inspect_classification_target(
    ...
    report_lines.append("")
    report_lines.append(confusion.to_markdown(index=False))
    report_lines.append("")

    if not fi_df.empty:
        report_lines.append(f"## Top feature drivers ({fi_kind})")
        report_lines.append("")
        report_lines.append(fi_df.head(12).to_markdown(index=False))
        report_lines.append("")
    ...
    if len(hard_errors) > 0:
        ...
        report_lines.append(hard_errors[cols].head(10).to_markdown(index=False))
        report_lines.append("")
    ...
    if len(top_correct) > 0:
        ...
        report_lines.append(top_correct[cols].head(10).to_markdown(index=False))
        report_lines.append("")
...
def inspect_regression_target(
    ...
    if len(worst) > 0:
        ...
        report_lines.append(worst[cols].head(10).to_markdown(index=False))
        report_lines.append("")
...
def inspect_one_target(project_root: Path, model_root: Path, reports_root: Path, out_root: Path, space: str, target: str) -> Optional[Dict]:
    ...
    # Copy useful originals into the inspection folder for convenience.
