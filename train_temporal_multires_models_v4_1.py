...
def focal_bce_with_logits(
    ...
    focal = (1.0 - pt).clamp_min(0.0).pow(gamma)
    return focal * bce
...
def build_train_sampler(
    ...
    train_idx, _, _ = split_dataset_indices(dataset)
    if target_name not in dataset.binary_targets:
        return None
    y = dataset.binary_targets[target_name][train_idx]
    m = dataset.binary_masks[target_name][train_idx] > 0
    if m.sum() == 0:
        return None
    yv = y[m]
    pos = float(yv.sum())
    ...
    sample_weights[valid_indices] = np.where(yv > 0.5, pos_w, 1.0).astype(np.float32)
...
class ModalityEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor, age_days: torch.Tensor) -> torch.Tensor:
        ...
        age_feat = torch.nan_to_num(age_days.unsqueeze(-1), nan=0.0, posinf=0.0, neginf=0.0)
...
def build_datasets(
    ...
    enabled_modalities: Optional[set[str]] = None,
    ...
    enabled_modalities = enabled_modalities or {"days", "meals", "weeks"}
    for name in list(modalities.keys()):
        if name not in enabled_modalities:
            modalities[name]["X"] = np.zeros_like(modalities[name]["X"], dtype=np.float32)
            modalities[name]["mask"] = np.zeros_like(modalities[name]["mask"], dtype=np.float32)
            modalities[name]["age_days"] = np.zeros_like(modalities[name]["age_days"], dtype=np.float32)
    ...
    if "meals" not in enabled_modalities:
        static_feats["has_meals"] = 0.0
        static_feats["n_meals_steps_observed"] = 0.0
    if "days" not in enabled_modalities:
        static_feats["has_days"] = 0.0
        static_feats["n_days_steps_observed"] = 0.0
    if "weeks" not in enabled_modalities:
        static_feats["has_weeks"] = 0.0
        static_feats["n_weeks_steps_observed"] = 0.0
    ...
    meta = {
        ...
        "enabled_modalities": sorted(list(enabled_modalities)),
    }
    return dataset, meta
...
def make_loaders(
    ...
    train_loader = DataLoader(
        ...
        num_workers=num_workers, pin_memory=pin_memory, collate_fn=collate_batch, drop_last=False,
    )
...
def move_batch_to_device(batch: Dict, device: torch.device) -> Dict:
    out = {
        ...
        "split": batch["split"],
        "static_feats": _sanitize_tensor(batch["static_feats"].to(device, non_blocking=True)),
        "binary_targets": {},
        "binary_masks": {},
        "regression_targets": {},
        "regression_masks": {},
        "days_x": _sanitize_tensor(batch["days_x"].to(device, non_blocking=True)),
        "days_mask": _sanitize_tensor(batch["days_mask"].to(device, non_blocking=True)),
        "days_age": _sanitize_tensor(batch["days_age"].to(device, non_blocking=True)),
        "meals_x": _sanitize_tensor(batch["meals_x"].to(device, non_blocking=True)),
        "meals_mask": _sanitize_tensor(batch["meals_mask"].to(device, non_blocking=True)),
        "meals_age": _sanitize_tensor(batch["meals_age"].to(device, non_blocking=True)),
        "weeks_x": _sanitize_tensor(batch["weeks_x"].to(device, non_blocking=True)),
        "weeks_mask": _sanitize_tensor(batch["weeks_mask"].to(device, non_blocking=True)),
        "weeks_age": _sanitize_tensor(batch["weeks_age"].to(device, non_blocking=True)),
    }
    for k in batch["binary_targets"]:
        out["binary_targets"][k] = _sanitize_tensor(batch["binary_targets"][k].to(device, non_blocking=True))
        out["binary_masks"][k] = _sanitize_tensor(batch["binary_masks"][k].to(device, non_blocking=True))
    for k in batch["regression_targets"]:
        out["regression_targets"][k] = _sanitize_tensor(batch["regression_targets"][k].to(device, non_blocking=True))
        out["regression_masks"][k] = _sanitize_tensor(batch["regression_masks"][k].to(device, non_blocking=True))
    return out
...
def evaluate_predictions(
    ...
    for t in binary_targets:
        ...
        m["threshold"] = thr
        m.update(prediction_distribution(prob))
        m["positive_rate_pred_at_0_5"] = float(np.mean(prob >= 0.5))
        m["positive_rate_pred_at_threshold"] = float(np.mean(prob >= thr))
        metrics["binary"][t] = m
...
def diagnostics_frame(
    ...
    for t in binary_targets:
        ...
        rows.append({
            "target": t,
            **prediction_distribution(prob),
            "event_rate_true": float(np.mean(y_true)),
            "positive_rate_pred_at_0_5": float(np.mean(prob >= 0.5)),
            "positive_rate_pred_at_threshold": float(np.mean(prob >= thr)),
            "threshold": thr,
            "n": int(len(prob)),
        })
...
def run_epoch(
    ...
    for batch_cpu in loader:
        ...
        with torch.amp.autocast(device_type=device.type, enabled=use_amp and device.type == "cuda"):
            ...
            loss, parts = compute_loss(
                ...
                device=device,
                binary_loss_mode=binary_loss_mode,
    ...
    for k, v in part_sums.items():
        out[f"train__{k}"] = v / max(n_batches, 1)
    return out
...
def make_grad_scaler(device: torch.device, use_amp: bool):
    ...
    if hasattr(torch, "amp") and hasattr(torch.amp, "GradScaler"):
        # Newer signatures may want a positional device string; older ones may not.
        try:
...
def main() -> None:
    ...
    parser.add_argument("--modalities", default="days,weeks,meals", help="Comma-separated modalities to enable.")
    ...
    parser.add_argument("--balanced-sampler", action="store_true", help="Use a weighted sampler for single-head binary runs.")
    parser.add_argument("--grad-clip", type=float, default=0.5)
    parser.add_argument("--num-workers", type=int, default=0)
    parser.add_argument("--seed", type=int, default=RANDOM_STATE)
    parser.add_argument("--amp", action="store_true")
    parser.add_argument("--smoke-test", action="store_true", help="Use safer defaults for short diagnostic runs.")
    parser.add_argument("--force-amp", action="store_true", help="Allow AMP even during smoke tests.")
    ...
    if args.smoke_test and use_amp and not args.force_amp:
        log("Disabling AMP for smoke test to reduce diagnostic instability.")
        use_amp = False
    if use_amp and args.model_family == "gru":
        log("Disabling AMP for GRU to avoid recurrent mixed-precision instability.")
        use_amp = False
    ...
    binary_targets = _parse_target_list(args.binary_targets)
    regression_targets = _parse_target_list(args.regression_targets)
    ...
    train_sampler = None
    if args.balanced_sampler and len(binary_targets) == 1 and len(regression_targets) == 0:
        train_sampler = build_train_sampler(dataset, binary_targets[0])
        if train_sampler is not None:
            log(f"Using balanced sampler for target: {binary_targets[0]}")
    ...
    config = {
        ...
        "device": str(device),
        "model_family": args.model_family,
        "modalities": sorted(list(enabled_modalities)),
        "hidden_dim": args.hidden_dim,
        "num_layers": args.num_layers,
        "dropout": args.dropout,
        "batch_size": args.batch_size,
        ...
        "binary_loss_mode": args.binary_loss_mode,
    ...
    for epoch in range(start_epoch, args.max_epochs):
        if args.warmup_epochs > 0 and epoch < args.warmup_epochs:
            warm_scale = float(epoch + 1) / float(args.warmup_epochs)
            for pg in optimizer.param_groups:
                pg["lr"] = base_learning_rate * warm_scale
        else:
            for pg in optimizer.param_groups:
                pg["lr"] = min(pg["lr"], base_learning_rate)
        ...
        # Evaluate at both fixed 0.5 and tuned-on-val thresholds.
        ...
        row = {
            ...
            "train_batches": train_stats.get("train_batches", 0),
        ...
        log(f"{run_name} epoch {epoch+1}/{args.max_epochs} train={train_stats['train_loss']:.4f} batches={train_stats.get('train_batches',0)} skipped={train_stats.get('skipped_nonfinite_batches',0)} val_score={score:.4f} best={best_score:.4f}")
    ...
    val_metrics_fixed = evaluate_predictions(
        preds=val_preds,
        binary_targets=binary_targets,
        regression_targets=regression_targets,
        tuned_thresholds={},
    )
    val_metrics_tuned = evaluate_predictions(
        preds=val_preds,
        binary_targets=binary_targets,
        regression_targets=regression_targets,
    ...
    diagnostics_frame(val_preds, binary_targets, best_thresholds).to_csv(report_dir / "val_prediction_diagnostics.csv", index=False)
    diagnostics_frame(test_preds, binary_targets, best_thresholds).to_csv(report_dir / "test_prediction_diagnostics.csv", index=False)
    ...
    summary_rows = []
    for t, m in test_metrics_fixed["binary"].items():
        summary_rows.append({
            "target": t,
            "kind": "binary_classification_fixed_0_5",
            "threshold": 0.5,
            **m,
        })
    for t, m in test_metrics_tuned["binary"].items():
        summary_rows.append({
            "target": t,
            "kind": "binary_classification_tuned",
            "threshold": best_thresholds.get(t, 0.5),
            **m,
        })
    for t, m in test_metrics_tuned["regression"].items():
        summary_rows.append({
            "target": t,
            "kind": "regression",
            **m,
