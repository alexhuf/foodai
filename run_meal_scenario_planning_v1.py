...
DEFAULT_RUN_NAME = "meal_scenario_planning_v1"
...
def build_summary(
    ...
    lines = [
        ...
        "## Action Space",
        "",
        f"- observed templates after required-slot filter: `{action_metadata['templates_after_required_slot_filter']}`",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Search observed meal-template scenarios over bounded horizons.")
    ...
    day_actions, action_metadata = build_day_action_library(tables)
    rankings, plan_details, stress = build_scenario_search(
        actions=day_actions,
        context=context,
    ...
    (report_dir / "summary.md").write_text(
        build_summary(rankings, plan_details, context_payload, action_metadata, report_dir),
        encoding="utf-8",
    )
    ...
    if not promoted.empty:
        ...
        print(best[["horizon_days", "plan_id", "robust_score", "robust_weight_support", "fragility"]].to_string(index=False))
