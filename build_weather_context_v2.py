...
def classify_temp_band_f(temp_f: pd.Series) -> pd.Series:
    ...
    labels = ["extreme_cold", "freezing", "cold", "cool", "mild", "warm", "hot"]
...
def add_hourly_features(hourly: pd.DataFrame) -> pd.DataFrame:
    ...
    h["apparent_temp_band_f"] = classify_temp_band_f(h["apparent_temperature"])
    ...
    h["is_dark_hour_raw"] = h["is_day"].fillna(1) == 0
    return h


def add_daily_features(daily: pd.DataFrame, solar_hour_offset: float) -> pd.DataFrame:
    d = daily.copy()

    d["sunrise_raw"] = pd.to_datetime(d["sunrise"], errors="coerce")
    d["sunset_raw"] = pd.to_datetime(d["sunset"], errors="coerce")

    offset = pd.to_timedelta(solar_hour_offset, unit="h")
    d["sunrise"] = d["sunrise_raw"] + offset
    d["sunset"] = d["sunset_raw"] + offset
    ...
    d["apparent_temp_band_f_mean"] = classify_temp_band_f(d["apparent_temperature_mean"])
...
def add_streaks(daily: pd.DataFrame) -> pd.DataFrame:
    ...
    d["snow_depth_gt_0"] = d["snowfall_sum"].fillna(0) > 0
    return d


def build_weather_context(project_root: Path, latitude: float, longitude: float, timezone: str, solar_hour_offset: float) -> None:
    ...
    daily_fused = pd.read_csv(daily_path, low_memory=False)
    intraday_fused = pd.read_csv(intraday_path, low_memory=False)

    daily_fused["date"] = pd.to_datetime(daily_fused["date"], errors="coerce").dt.floor("D")
    intraday_fused["datetime_local"] = pd.to_datetime(intraday_fused["datetime_local"], errors="coerce")

    start_date = daily_fused["date"].min().strftime("%Y-%m-%d")
    end_date = daily_fused["date"].max().strftime("%Y-%m-%d")
    ...
    daily_weather = add_daily_features(daily_weather, solar_hour_offset)
    daily_weather = add_streaks(daily_weather)

    # align hourly -> 15 min
    hourly_idx = hourly.set_index("datetime_local").sort_index()
    target_index = pd.date_range(
        start=intraday_fused["datetime_local"].min(),
        end=intraday_fused["datetime_local"].max(),
        freq="15min",
    )
    hourly_15 = hourly_idx.reindex(target_index.union(hourly_idx.index)).sort_index().ffill()
    ...
    # corrected is_day from corrected sunrise/sunset
    daily_solar = daily_weather[["date", "sunrise", "sunset", "sunrise_raw", "sunset_raw"]].copy()
    intraday_weather = intraday_fused[["datetime_local"]].merge(hourly_15, on="datetime_local", how="left")
    intraday_weather["date"] = intraday_weather["datetime_local"].dt.floor("D")
    intraday_weather = intraday_weather.merge(daily_solar, on="date", how="left")

    intraday_weather["is_day_corrected"] = (
        (intraday_weather["datetime_local"] >= intraday_weather["sunrise"]) &
        (intraday_weather["datetime_local"] < intraday_weather["sunset"])
    )
    intraday_weather["is_dark_hour"] = ~intraday_weather["is_day_corrected"]
    intraday_weather["solar_hour_offset_applied"] = solar_hour_offset

    intraday_weather = intraday_weather.merge(
        daily_weather.drop(columns=["sunrise_raw", "sunset_raw"]),
        on="date",
        how="left",
        suffixes=("", "_daily"),
    )

    daily_weather_aligned = daily_fused[["date"]].merge(daily_weather, on="date", how="left")
    daily_weather_aligned["solar_hour_offset_applied"] = solar_hour_offset
    ...
    metadata = {
        ...
        "solar_hour_offset_applied": solar_hour_offset,
        "note": "Corrected sunrise/sunset and derived corrected is_day from daily solar times.",
    }

    log("Writing corrected weather datasets...")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build corrected historical weather/daylight context aligned to fused timeline.")
    ...
    parser.add_argument("--solar-hour-offset", type=float, default=-1.0, help="Hour offset applied to sunrise/sunset and corrected is_day.")
    ...
    build_weather_context(
        ...
        solar_hour_offset=args.solar_hour_offset,
    )
