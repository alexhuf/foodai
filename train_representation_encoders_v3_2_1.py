...
def train_space(
    ...
    run_dir = project_root / "models" / "representation_v3_2_1" / space
    ensure_dir(run_dir)

    log(f"Loading source data for {space} ...")
    df = pd.read_csv(source_csv, low_memory=False)
    manifest = load_json(manifest_json)
    feature_cols_manifest = [c for c in manifest["feature_columns"] if c in df.columns]
    X_raw = build_transform_frame(
        manifest_cols=feature_cols_manifest,
        df=df,
        neutralize_cols=cfg.get("neutralize_input_cols", []),
    )
    id_col = cfg["id_col"]
    ...
    run_config = {
        ...
        "feature_cols_manifest": feature_cols_manifest,
        "neutralized_input_cols": cfg.get("neutralize_input_cols", []),
...
def main():
    parser = argparse.ArgumentParser(description="Train representation encoders v3.2.1 with leakage-safe regime inputs.")
    ...
    ensure_dir(project_root / "models" / "representation_v3_2_1")
