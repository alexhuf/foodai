...
def build_repaired_final(project_root: Path, meal_db_dir: str = "meal_db") -> None:
    ...
    for col in semantic_alias_cols:
        ...
        if review_col in alias_master.columns:
            alias_master[col] = alias_master[review_col].combine_first(series_or_na(alias_master, col))
    ...
    if "component_role_suggested" in comp_master.columns:
        ...
        comp_master["component_role_final"] = series_or_na(comp_master, "component_role").combine_first(series_or_na(comp_master, "heuristic_role_hint"))
    if "role_confidence_suggested" in comp_master.columns:
        comp_master["role_confidence"] = comp_master["role_confidence_suggested"].combine_first(series_or_na(comp_master, "role_confidence"))
    ...
    for base_col in [
        ...
        if explicit_col in event_final.columns:
            event_final[base_col] = event_final[explicit_col].combine_first(series_or_na(event_final, base_col))
        if fallback_col in event_final.columns:
            event_final[base_col] = series_or_na(event_final, base_col).combine_first(event_final[fallback_col])
