...
def build_transition_table(df: pd.DataFrame, cfg: SpaceConfig, horizon: int = 1) -> pd.DataFrame:
    ...
    work["transition_horizon"] = horizon
    ...
    # Next-period raw copies for key target columns
    ...
    for col in available_categorical:
        work[f"y_next_{col}"] = work[col].shift(-horizon)
        work[f"y_same_{col}"] = (
            work[col].astype("object").where(work[col].notna(), None)
            == work[f"y_next_{col}"].astype("object").where(work[f"y_next_{col}"].notna(), None)
        )
        work.loc[work[col].isna() | work[f"y_next_{col}"].isna(), f"y_same_{col}"] = np.nan

    # Derived forward-looking binary targets that are easier to train / rank on.
    # These are intentionally simple domain targets.
    if "y_next_weight_delta_lb" in work.columns:
        work["y_next_weight_loss_flag"] = work["y_next_weight_delta_lb"] <= -0.5
        work["y_next_weight_gain_flag"] = work["y_next_weight_delta_lb"] >= 0.5
    ...
    if restaurant_col:
        work["y_next_restaurant_heavy_flag"] = work[restaurant_col] >= 0.50
    ...
    if budget_col:
        work["y_next_budget_breach_flag"] = work[budget_col] < 0
    ...
    if meal_freq_col:
        work["y_next_high_meal_frequency_flag"] = work[meal_freq_col] >= 4.0

    # Remove rows with no next-period target.
    out = work[work["next_period_id"].notna()].copy().reset_index(drop=True)

    # Chronological suggested split based on current period.
    out["split_suggested"] = temporal_split_labels(len(out))

    # A small set of convenience metadata columns.
    out["is_gap_expected"] = out["gap_vs_expected_days"].abs() <= 1.5
...
def summarize_transition_table(df: pd.DataFrame, cfg: SpaceConfig) -> Dict:
    ...
    binary_y = [c for c in y_cols if set(df[c].dropna().astype(str).unique().tolist()).issubset({"True", "False", "0", "1"})]
    summary = {
        "space": cfg.name,
        ...
        "pct_gap_expected": None if len(df) == 0 else float(df["is_gap_expected"].mean()),
        "target_column_count": len(y_cols),
        "numeric_target_count": len(numeric_y),
        "binary_like_target_count": len(binary_y),
        "split_counts": df["split_suggested"].value_counts(dropna=False).to_dict(),
    }
    return summary
